...
from flask_jwt_extended import create_access_token, set_access_cookies, jwt_required, get_jwt_identity, unset_jwt_cookies, create_refresh_token, decode_token, get_jwt
from jwt.exceptions import ExpiredSignatureError, DecodeError as JWTDecodeError
import stripe
import os
#------------------------------------HELPERS----------------------------------
...
def send_rental_confirmation_email(user, owner, equipment, payment_intent, cart_item):
    ...
    if not all([email_sender, email_password]):
        print("❌ Email Error: EMAIL_SENDER or EMAIL_PASSWORD not set in .env file.")
        return

    # Set the recipient to the user's email
    email_receiver = user.email

    # Format a professional-looking HTML email body
    subject = f"✅ Your EquipMe Rental Confirmation for {equipment.name}"
    body = f"""
    <html>
        <head>
            <style>
        ...
        <body>
            <div class="container">
                ...
                <div class="content">
                    ...
                    <p>Thank you for your rental through EquipMe. Your payment has been successfully processed and your rental is confirmed.</p>
                    <hr>
                    <h4>Rental Details:</h4>
                    <ul>
                        <li><b>Equipment:</b> {cart_item.quantity} x {equipment.make} {equipment.model}</li>
                        <li><b>Owner:</b> {owner.firstName} {owner.lastName}</li>
                        <li><b>Rental Period:</b> {cart_item.rental_length} {cart_item.rental_rate.replace('ly', '')}(s)</li>
                    </ul>
                    <h4>Payment Details:</h4>
                    <ul>
                        <li><b>Payment ID:</b> {payment_intent.id}</li>
                        <li><b>Amount Paid:</b> ${(payment_intent.amount_received / 100):.2f} {payment_intent.currency.upper()}</li>
                    </ul>
                    <hr>
                    <p>You can view your order details in your dashboard. If you have any questions, please contact the equipment owner directly.</p>
                    <p>Thank you for choosing EquipMe!</p>
                </div>
                <div class="footer">
                    <p>&copy; {datetime.now().year} EquipMe. All rights reserved.</p>
                </div>
            </div>
        </body>
    </html>
    ...
    try:
        ...
        print(f"✅ Confirmation email successfully sent to {email_receiver}")
...
class Login(Resource):
    def post(self):
        ...
        if user and user.authenticate(password):
            ...
            # https://flask-jwt-extended.readthedocs.io/en/stable/api.html#flask_jwt_extended.create_access_token It's made JSON serializable with 'This can be accessed later with []' : other_value
        ...
        if owner and owner.authenticate(password):
            ...
            access_token = create_access_token(identity=str(owner.id), additional_claims={'role': 'owner'})
...
class CheckSession(Resource):
    @jwt_required()
    def get(self):
        # Get the user's unique string ID from the token's identity
        identity_id = get_jwt_identity()
        
        # Get the entire token's payload to access the 'role' from additional claims
        claims = get_jwt()
        identity_role = claims.get('role')
...
# Add this new class to your app.py file

class GetCheckoutSessionDetails(Resource):
    def get(self, session_id):
        """
        Retrieves checkout session details directly from Stripe's API.
        This provides immediate, reliable confirmation of payment status.
        """
        try:
            # Use the Stripe library to retrieve the session object
            checkout_session = stripe.checkout.Session.retrieve(session_id)
            
            # The session object contains all relevant details, such as:
            # - status ('complete', 'open', 'expired')
            # - payment_intent (ID of the payment)
            # - amount_total (in cents)
            # - currency
            # See Stripe docs for the full object: https://stripe.com/docs/api/checkout/sessions/object
            
            # Return the entire session object to the frontend
            return make_response(checkout_session, 200)

        except stripe.error.InvalidRequestError as e:
            # This can happen if the session_id is invalid
            print(f"Stripe Error: {e}")
            return make_response({'error': 'Invalid session ID'}, 404)
        except Exception as e:
            # Catch any other unexpected errors
            print(f"An unexpected error occurred: {e}")
            return make_response({'error': 'An internal server error occurred'}, 500)

# Add the new resource to your API. Make sure this line is also in your app.py
api.add_resource(GetCheckoutSessionDetails, '/checkout-session/<string:session_id>')
...
# In app.py, replace your entire WebHookForStripeSuccess class with this one.

class WebHookForStripeSuccess(Resource):
    def post(self):
        # It's recommended to get the webhook secret from environment variables
        endpoint_secret = os.getenv('WEBHOOK_SECRET')
        if not endpoint_secret:
            print("❌ Webhook Error: WEBHOOK_SECRET environment variable not set.")
        ...
        # Handle the checkout session completion event (This is the main trigger)
        if event['type'] == 'checkout.session.completed':
            print("🔔 Webhook Info: Received 'checkout.session.completed' event.")
            checkout_session = event['data']['object']
            
            # The checkout session contains the payment_intent ID. We need to retrieve the full object.
            try:
                payment_intent = stripe.PaymentIntent.retrieve(checkout_session.payment_intent)
                print(f"  - Retrieved Payment Intent: {payment_intent.id}")
                # Now, call the existing handler with the full payment_intent object
                self.handle_successful_payment_intent(payment_intent)
            except Exception as e:
                print(f"❌ Webhook Error: Could not retrieve Payment Intent {checkout_session.payment_intent}. Error: {e}")

        # Keep the original handler as a fallback
        elif event['type'] == 'payment_intent.succeeded':
            print("🔔 Webhook Info: Received 'payment_intent.succeeded' event.")
            self.handle_successful_payment_intent(event['data']['object'])
        
        else:
            print(f'🔔 Webhook Info: Received unhandled event type: {event["type"]}')
    ...
    def handle_successful_payment_intent(self, payment_intent):
        # This entire method remains the same as the one I provided previously.
        # It contains all the logic for updating the database.
        print("\n✅ --- Webhook: Handling successful payment intent ---")
        print(f"Payment Intent ID: {payment_intent.get('id')}")

        metadata = payment_intent.get("metadata", {})
        equipment_ids_str = metadata.get("equipment_ids")
        cart_item_ids_str = metadata.get("cart_item_ids")
        user_ids_str = metadata.get("user_ids")

        if not all([equipment_ids_str, cart_item_ids_str, user_ids_str]):
            print("❌ Webhook Error: Missing required metadata (equipment_ids, cart_item_ids, or user_ids).")
            return

        print(f"Received Metadata -> User IDs: {user_ids_str}, Equipment IDs: {equipment_ids_str}, Cart Item IDs: {cart_item_ids_str}")
        ...
        for equipment_id, cart_item_id, user_id in zip(equipment_ids, cart_item_ids, user_ids):
            print(f"\nProcessing item -> User: {user_id}, Equipment: {equipment_id}, Cart Item: {cart_item_id}")
            
            try:
                # Use with_for_update() to lock the rows for the duration of the transaction
                equipment = Equipment.query.with_for_update().get(equipment_id)
                cart_item = CartItem.query.with_for_update().get(cart_item_id)
                user = User.query.get(user_id)
                agreement = RentalAgreement.query.filter(RentalAgreement.cart_item_id == cart_item_id).with_for_update().first()

                if not all([equipment, cart_item, user, agreement]):
                    print(f"❌ Webhook Error: Could not find all required DB records for this item. Skipping.")
                    continue

                agreement.agreement_status = "completed"
                print(f"  - Agreement {agreement.id} status set to 'completed'.")

                equipment_status = equipment.status[0]
                if equipment_status.reserved_quantity >= cart_item.quantity:
                    equipment_status.reserved_quantity -= cart_item.quantity
                    equipment_status.rented_quantity += cart_item.quantity
                    print(f"  - Equipment {equipment.id} status updated: reserved -{cart_item.quantity}, rented +{cart_item.quantity}.")
                else:
                    print(f"❌ Webhook Warning: Not enough reserved quantity for equipment {equipment.id}. Skipping status update.")
                    continue

                last_state = EquipmentStateHistory.query.filter_by(equipment_id=equipment.id).order_by(EquipmentStateHistory.changed_at.desc()).first()
                new_state_history = EquipmentStateHistory(
                    equipment_id=equipment.id,
                    total_quantity=last_state.total_quantity,
                    available_quantity=last_state.available_quantity,
                    reserved_quantity=equipment_status.reserved_quantity,
                    rented_quantity=equipment_status.rented_quantity,
                    previous_state=last_state.new_state,
                    new_state=f'{user.firstName} {user.lastName} has rented {cart_item.quantity} {equipment.make} {equipment.model}',
                    changed_at=datetime.utcnow()
                )
                db.session.add(new_state_history)
                print("  - New equipment state history record created.")
                ...
                if payment_method_id:
                    try:
                        ...
                        if payment_method_obj.type == 'card':
                            card = payment_method_obj.card
                            payment_method_details = f'{card.brand.title()}: **** {card.last4}'
                    except Exception as e:
                        print(f"  - Warning: Could not retrieve payment method details. Error: {e}")

                new_order_history = OrderHistory(
                    order_datetime=datetime.utcnow(),
                    total_amount=payment_intent.get('amount_received'),
                ...
                print("  - New order history record created.")
                ...
                print(f"  - New payment record created for intent {payment_intent.id}.")

                db.session.commit()
                print(f"✅ Successfully processed and committed item to DB.")

                # After a successful commit, send the confirmation email
                send_rental_confirmation_email(user=user, owner=equipment.owner, equipment=equipment, payment_intent=payment_intent, cart_item=cart_item)

            except Exception as e:
                db.session.rollback()
                print(f"❌❌❌ An unexpected error occurred while processing an item: {e}. Rolling back changes for this item.")

# Make sure this line is present in your app.py to register the resource
...
#https://www.youtube.com/watch?v=g_j6ILT-X0

# 1. A new, more robust and reusable email sending function
def send_email(subject, body, recipient, is_html=False):
    """
    A general-purpose function to send emails.
    - subject: The email subject line.
    - body: The email content (can be plain text or HTML).
    - recipient: The email address of the person to send the email to.
    - is_html: Set to True if the body is HTML content.
    """
    email_sender = os.getenv('EMAIL_SENDER')
    email_password = os.getenv('EMAIL_PASSWORD')
...
# 2. The updated ContactFormSubmission class
class ContactFormSubmission(Resource):
    def post(self):
        ...
        if not all([name, email, subject, message]):
            return make_response(jsonify({"error": "Missing required form fields"}), 400)

        # --- Email to the Site Admin ---
        admin_email = "bispo.swe@gmail.com" # Your admin email
        admin_subject = f"New Contact Form Submission: {subject}"
        admin_body = f"""
        You have a new message from the EquipMe contact form:
        -------------------------------------------------
        Name: {name}
        Email: {email}
        Subject: {subject}
        ...
        {message}
        """
        admin_email_sent = send_email(admin_subject, admin_body, admin_email)

        # --- Auto-Reply Email to the User ---
        user_subject = "Thank you for contacting EquipMe!"
        user_body = f"""
        Hello {name},
        ...
        The EquipMe Team
        """
        user_email_sent = send_email(user_subject, user_body, email)

        if admin_email_sent and user_email_sent:
            return make_response(jsonify({"message": "Your message has been sent successfully!"}), 200)
        else:
            return make_response(jsonify({"error": "There was an issue sending your message. Please try again later."}), 500)

# 3. Make sure the resource is registered with the API
...
class RequestPasswordReset(Resource):
    def post(self):
        ...
        owner = EquipmentOwner.query.filter_by(email=email).first()
        
        target_user = user or owner
        
        if target_user:
            role = 'user' if isinstance(target_user, User) else 'owner'
            
            # --- CORRECTED TOKEN CREATION ---
            # The identity MUST be a string. All other data goes in additional_claims.
            reset_token = create_access_token(
                identity=str(target_user.id),
                additional_claims={'role': role, 'purpose': 'password_reset'},
                expires_delta=timedelta(minutes=15)
            )
            
            reset_url = f"http://localhost:3000/reset-password/{reset_token}"
            
            subject = "Your Password Reset Link for EquipMe"
            body = f"""
            <p>Hello {target_user.firstName},</p>
            <p>You requested a password reset. Please click the link below to set a new password. This link is valid for 15 minutes.</p>
            <p><a href="{reset_url}" style="background-color: #4CAF50; color: white; padding: 14px 25px; text-align: center; text-decoration: none; display: inline-block;">Reset Your Password</a></p>
            <p>If you did not request this, please ignore this email.</p>
            """
            send_email(subject, body, target_user.email, is_html=True)
...
class ResetPassword(Resource):
    def post(self):
        ...
        try:
            # decode_token gives us the full payload of the JWT
            decoded_token = decode_token(token)
            
            # --- CORRECTED TOKEN READING ---
            # The user's ID is the 'sub' (subject) claim.
            user_id = decoded_token['sub']
            
            # The additional claims are at the top level of the decoded token.
            if decoded_token.get('purpose') != 'password_reset':
                return make_response(jsonify({"error": "Invalid token purpose"}), 401)
            ...
            if role == 'user':
                ...
                target_user = EquipmentOwner.query.get(user_id)

            if not target_user:
                return make_response(jsonify({"error": "User not found"}), 404)

            # The setter in your model will hash the password automatically
            ...
            return make_response(jsonify({"error": "The password reset link has expired."}), 401)
        except (JWTDecodeError, Exception) as e:
            print(f"Password reset error: {e}")
            return make_response(jsonify({"error": "Invalid or malformed token."}), 401)

# --- 3. API RESOURCE REGISTRATION (Should already exist) ---
